...
const DockItem = ({ icon, label, href, active, onClick }: DockItemProps) => {
    const content = (
        ...
        <>
            <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-[100] w-auto max-w-[95vw]">
                <div className="flex items-center gap-1 px-4 py-3 bg-black/60 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl overflow-x-auto scrollbar-hide">
                    ...
                    <DockItem icon={<Briefcase size={20} className="text-purple-400" />} label="Consult" href="/dreamnova-consult" active={false} />
                    <DockItem icon={<Globe size={20} className="text-blue-400" />} label="Global" href="/dreamnova-global" active={false} />
                    <DockItem icon={<ShoppingCart size={20} className="text-orange-400" />} label="Ha-Mazon" href="/ha-mazon" active={false} />
                    <DockItem icon={<Brain size={20} className="text-pink-400" />} label="Tera" href="/tera-mind" active={false} />
                    <div className="w-px h-6 bg-white/10 mx-1"></div>
                    <DockItem icon={<BookOpen size={20} className="text-amber-400" />} label="Breslev" href="/breslev-books" active={false} />
                    <DockItem icon={<Music size={20} className="text-rose-400" />} label="Music" href="/tetrabrame" active={false} />
                    <DockItem icon={<GraduationCap size={20} className="text-emerald-400" />} label="Academy" href="/academy" active={false} />
                    <div className="w-px h-6 bg-white/10 mx-1"></div>
                    <DockItem icon={<TrendingUp size={20} className="text-yellow-400" />} label="Pitch" href="/pitch-deck" active={false} />
                    <DockItem icon={<Heart size={20} className="text-red-500" />} label="Foundation" href="/nova-foundation" active={false} />
                    <div className="w-px h-6 bg-white/10 mx-1"></div>
                    <DockItem
